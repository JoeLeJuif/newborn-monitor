...
// ─── LFO ──────────────────────────────────────────────────────────────────
// Architecture: LFO (−1…+1) → lfoGain (×depth) → filter.frequency AudioParam
//
// The GainNode is the single bypass control: gain=0 means exactly 0 added to
// filter.frequency regardless of the oscillator state — no connect/disconnect,
// no min/max manipulation, no residual values.
//
// Effective filter frequency = filter.frequency.value + (lfo_out × lfoGain)
//                            = baseCutoff + 0           (LFO off)
//                            = baseCutoff ± depth       (LFO on)
const lfo     = new Tone.LFO({ type: "sine", frequency: 2, min: -1, max: 1 });
const lfoGain = new Tone.Gain(0); // gain=0 → no modulation until enabled
lfo.connect(lfoGain);
...
// Tracks the user-set cutoff, always restored when LFO is disabled.
let baseCutoff = 8000;
...
export function setLFODepth(depth) {
  lfoDepth = depth;
  if (lfoActive) lfoGain.gain.value = depth;
}
...
export function setLFOEnabled(enabled) {
  lfoActive = enabled;
  if (enabled) {
    lfoGain.gain.value = lfoDepth;       // open the gate
  } else {
    lfoGain.gain.value = 0;              // close the gate — absolute zero
    filter.frequency.value = baseCutoff; // restore base cutoff explicitly
  }
}
